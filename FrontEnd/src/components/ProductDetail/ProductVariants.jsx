...
import { FiCpu, FiMonitor, FiBattery, FiSmartphone, FiCheckCircle } from "react-icons/fi";
...
const ProductVariants = ({ product = {}, selectedVariantId, onSelect }) => {
  ...
  const fixedSpecs = useMemo(() => {
    ...
    if (specMap.CPU || specMap.cpu || product.cpu) specs.push({ key: "Vi xử lý", value: specMap.CPU || specMap.cpu || product.cpu, icon: FiCpu });
    if (specMap.Screen || specMap.screen || product.screen) specs.push({ key: "Màn hình", value: specMap.Screen || specMap.screen || product.screen, icon: FiMonitor });
    if (specMap.Battery || specMap.battery || product.battery) specs.push({ key: "Dung lượng Pin", value: specMap.Battery || specMap.battery || product.battery, icon: FiBattery });
    if (specMap.OS || specMap.os || product.os) specs.push({ key: "Hệ điều hành", value: specMap.OS || specMap.os || product.os, icon: FiSmartphone });
    return specs;
  }, [product]);
  ...
  const attributeKeys = useMemo(() => {
    ...
    const priorityKeywords = ["ram", "rom", "storage", "capacity", "bộ nhớ", "ổ cứng", "dung lượng"];
  ...
  const selectedVariant = variants.find(v => String(v.id) === String(selectedVariantId)) || variants[0] || {};
  ...
  const handleApplyAttribute = (key, value) => {
    ...
    const findVariant = (targetAttrs) => {
      return variants.find(v => {
        ...
        const vAttrs = getAttrs(v);
        return Object.entries(targetAttrs).every(([k, vVal]) => normalize(vAttrs[k]) === normalize(vVal));
      });
    };
    let found = findVariant({ ...selectedAttributes, [key]: value });
    if (!found) found = findVariant(baseSelection);
    if (!found) found = variants.find(v => v.isActive && normalize(getAttrs(v)[key]) === normalize(value));
  ...
  const getStatus = (key, val) => {
    ...
    const matching = variants.filter(v => {
      if (!v.isActive) return false;
      const vAttrs = getAttrs(v);
      return normalize(vAttrs[key]) === normalize(val) &&
             Object.entries(baseSelection).every(([k, vV]) => normalize(vAttrs[k]) === normalize(vV));
    });
    if (matching.length === 0) return "disabled";
    return matching.some(v => Number(v.stock) > 0) ? "available" : "out-of-stock";
  };
    ...
    <div className="space-y-8 pt-6 border-t border-slate-100">
      ...
      {fixedSpecs.length > 0 && (
        <div className="space-y-4">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Cấu hình mặc định</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {fixedSpecs.map((spec, i) => (
              <div key={i} className="flex items-center gap-3 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl">
        ...
        <div key={key}>
          ...
          <div className="flex flex-wrap gap-3">
            {attributeValues[key].map((val) => {
              const active = normalize(selectedAttributes[key]) === normalize(val);
              const status = getStatus(key, val);
              const isDisabled = status === "disabled" || status === "out-of-stock";
              ...
              return (
                ...
                >
                  ...
                  {status === "out-of-stock" && !active && (
                    <span className="absolute -top-2 -right-2 bg-danger text-[8px] text-white px-1.5 py-0.5 rounded-lg shadow-sm z-20 font-black">HẾT</span>
                  )}
                  {active && (
                    <motion.div 
