...
const ReviewModal = ({ isOpen, onClose, order, onReviewSuccess }) => {
  const [reviewingItem, setReviewingProductId] = useState(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await createReviewApi({
        productId: reviewingItem,
        ...newReview
      });
      if (res.errCode === 0) {
        toast.success("Đánh giá sản phẩm thành công!");
        setReviewedProductIds(prev => [...prev, reviewingItem]);
        setReviewingProductId(null);
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
          {order.orderItems?.map((item) => {
            ...
            const isCurrent = reviewingItem === item.productId;

            return (
              <div key={item.id} className="bg-slate-50 rounded-[24px] p-5 border border-slate-100">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Số lượng: {item.quantity}</p>
                  </div>
                  ...
                  {isReviewed ? (
                    ...
                    <Button 
                      ...
                      onClick={() => handleOpenForm(item.productId)}
                    >
                      ĐÁNH GIÁ
                    </Button>
                  ) : (
                    <button onClick={() => setReviewingProductId(null)} className="text-[10px] font-bold text-slate-400 hover:text-red-500 uppercase tracking-widest px-2">Hủy</button>
                  )}
                </div>
