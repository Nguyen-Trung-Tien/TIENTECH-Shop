      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {items.map((item, idx) => {
          ...
          return (
            ...
            >
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 transition-transform duration-500 group-hover:scale-110 ${item.color}`}>
                <Icon size={24} />
              </div>
              <div>
                <h5 className="text-sm font-black text-surface-900 uppercase tracking-tight mb-1">{item.title}</h5>
                <p className="text-[12px] text-surface-400 font-medium leading-tight">{item.desc}</p>
              </div>
            </motion.div>
          );
        })}
