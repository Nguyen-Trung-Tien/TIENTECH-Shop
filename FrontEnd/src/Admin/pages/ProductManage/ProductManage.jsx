              ...
              {loadingTable ? Array(5).fill(0).map((_, i) => (
                ...
                <tr key={p.id} className="hover:bg-indigo-50/20 transition-all group">
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="min-w-0">
                        <p className="text-base font-bold text-slate-900 group-hover:text-indigo-600 transition-colors truncate max-w-[200px]">{p.name}</p>
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">SKU: {p.sku || "—"}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                  ...
                  <td className="px-8 py-5 text-right">
                    <p className="text-base font-black text-slate-900">{Number(p.basePrice).toLocaleString()} <span className="text-[10px] text-slate-400 font-bold uppercase ml-1">VND</span></p>
                    {p.isFlashSale && <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-orange-100 text-orange-600 text-[9px] font-black uppercase rounded mt-1"><FiZap size={10} /> Flash Sale</span>}
                  </td>
                  <td className="px-8 py-5 text-center">
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-xl border ${p.totalStock <= 5 ? "bg-rose-50 border-rose-100 text-rose-600" : "bg-emerald-50 border-emerald-100 text-emerald-600"}`}>
                      <FiPackage size={12} />
                      <span className="text-xs font-bold">{p.totalStock}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5 text-center">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${p.isActive ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-slate-100 text-slate-400 border-slate-200"}`}>
                      {p.isActive ? "Đang bán" : "Tạm ẩn"}
                    </span>
                  </td>
                  <td className="px-8 py-5 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => handleShowModal(p)} className="p-3 rounded-2xl text-slate-400 hover:bg-white hover:text-indigo-600 hover:shadow-xl border border-transparent transition-all"><FiEdit2 /></button>
                      <button onClick={() => setConfirmModal({ show: true, productId: p.id, name: p.name })} className="p-3 rounded-2xl text-slate-400 hover:bg-white hover:text-rose-500 hover:shadow-xl border border-transparent transition-all"><FiTrash2 /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-6xl max-h-[95vh] bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
                <form id="product-form" onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                       ...
                       <div className="group relative w-full aspect-square rounded-[2rem] bg-slate-50 border-2 border-dashed border-slate-200 flex flex-col items-center justify-center overflow-hidden transition-all hover:border-indigo-400 hover:bg-indigo-50/30 shadow-inner">
                          {imagePreview ? <img src={imagePreview} className="w-full h-full object-contain p-4" /> : (
                            <>
                              ...
                              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Chọn ảnh chính</span>
                            </>
                          )}
