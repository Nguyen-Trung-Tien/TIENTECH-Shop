      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <div className="bg-white border border-slate-200 p-1 rounded-2xl flex items-center shadow-sm">
            {["week", "month", "year"].map((p) => (
              <button
                ...
                className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${period === p ? "bg-indigo-600 text-white shadow-md shadow-indigo-200" : "text-slate-400 hover:text-slate-600"}`}
              >
          ...
          <button
            ...
            className="btn-modern-white h-11 px-5 group"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((card, i) => (
          <motion.div
            ...
            key={i}
            className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-500"
          >
            <div
            ...
            <h3 className="text-2xl font-black text-slate-900 mt-1">
              ...
              card.title.includes("doanh thu")
                ? card.value.toLocaleString() + "₫"
                : card.value.toLocaleString()}
            </h3>
            <div className="flex items-center gap-2 mt-4">
              <span
                className={`flex items-center gap-1 text-[10px] font-black px-2 py-0.5 rounded-lg ${card.isUp ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-10 pb-6 border-b border-slate-50">
            ...
            <div className="flex items-center gap-2 text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full text-[10px] font-black tracking-widest">
