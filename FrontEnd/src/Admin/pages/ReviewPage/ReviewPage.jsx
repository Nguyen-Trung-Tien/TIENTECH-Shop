      ...
      <div className="space-y-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32">
             ...
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Đang tải đánh giá...</p>
          </div>
        ) : reviews.length === 0 ? (
          <div className="bg-white rounded-[40px] border border-slate-100 p-20 text-center shadow-soft">
             <p className="text-slate-400 font-bold">Chưa có đánh giá nào phù hợp với bộ lọc.</p>
          </div>
        ) : (
          reviews.map((review) => (
            <motion.div 
              ...
              <div className="p-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-base font-black text-slate-900 leading-tight mb-1">{review.user?.username || "Unknown User"}</h4>
                      <p className="text-[10px] font-black text-primary uppercase tracking-widest">{review.product?.name}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-1">
                       {[...Array(5)].map((_, i) => (
                         <FaStar key={i} className={i < review.rating ? "text-amber-400" : "text-slate-200"} size={14} />
                       ))}
                    </div>
                    <button 
                ...
                <div className="space-y-4 ml-6 border-l-2 border-slate-100 pl-6">
                  {(replies[review.id] || []).map((rep) => (
                    <div key={rep.id} className="bg-slate-50/30 p-4 rounded-2xl relative group/reply">
                       <FiCornerDownRight className="absolute -left-8 top-5 text-slate-200" />
                       <div className="flex justify-between items-start gap-4">
                          <p className="text-sm text-slate-600">
                             <span className="font-black text-slate-900 mr-2 uppercase text-[10px] tracking-widest bg-slate-100 px-2 py-0.5 rounded">Admin</span>
                             {rep.comment}
                          </p>
                          <button 
                  ...
                  <div className="relative pt-2">
                    <input
                      ...
                      onKeyDown={(e) => e.key === "Enter" && handleCreateReply(review.id)}
