...
const OrdersCancelManage = () => {
  ...
  const [confirmModal, setConfirmModal] = useState({ show: false, approve: true });
  const limit = 10;
            ...
            {orders.map((order) => (
              ...
              >
                <div className="p-5 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
                   <span className="text-xs font-bold text-slate-900 bg-white px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm">#{order.orderCode}</span>
                   <StatusBadge map={paymentStatusMap} status={order.paymentStatus} className="text-[10px]" />
                </div>
