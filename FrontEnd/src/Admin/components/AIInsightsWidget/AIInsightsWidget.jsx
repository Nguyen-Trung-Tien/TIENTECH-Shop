...
import { FiCpu, FiTrendingUp, FiAlertCircle, FiShoppingBag, FiStar, FiChevronDown, FiChevronUp } from "react-icons/fi";
    ...
    <div className="bg-slate-950 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden transition-all duration-500">
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-lg transition-all ${isExpanded ? 'bg-primary text-white scale-110 shadow-primary/20' : 'bg-slate-800 text-slate-500'}`}>
            <FiCpu className={isExpanded ? "animate-pulse" : ""} />
          </div>
          <div>
            <h3 className="text-base font-black text-white tracking-tight uppercase">AI Business Consultant</h3>
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Gợi ý chiến lược thông minh từ TienTech AI</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          {!isExpanded && (
            <span className="hidden md:inline text-[9px] font-black text-slate-500 bg-slate-900 px-3 py-1 rounded-full uppercase tracking-widest border border-slate-800">Tắt - Nhấn để mở</span>
          )}
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center bg-white/5 text-slate-400 transition-transform ${isExpanded ? "rotate-0" : "rotate-180"}`}>
            <FiChevronUp />
          </div>
        </div>
      </div>
          ...
          >
            {loading ? (
              <div className="p-12 flex flex-col items-center gap-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Đang phân tích dữ liệu hệ thống...</p>
              </div>
            ) : insights ? (
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-slate-800">
                ...
                <div className="md:col-span-2 border-t border-slate-800/50 pt-6 mt-2">
                   <div className="bg-primary/5 border border-primary/10 rounded-2xl p-6 flex items-start gap-4">
                      ...
                      <div>
                         <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-1">Lời khuyên chiến lược</p>
              ...
              <div className="p-8 text-center text-slate-500 text-xs italic">Không có dữ liệu phân tích.</div>
            )}
          </motion.div>
        )}
      </AnimatePresence>
